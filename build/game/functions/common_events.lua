...
function update_canvas_juice(dt)
    -- Inicializuj WINDOWTRANS pokud neexistuje (LovePotion fallback)
    if not G.WINDOWTRANS then
        G.WINDOWTRANS = {
            real_window_w = love.graphics.getWidth() or 1280,
            real_window_h = love.graphics.getHeight() or 720
        }
    end
    
    -- Inicializuj ROOM_ORIG pokud neexistuje (LovePotion fallback)
    if not G.ROOM_ORIG then
        G.ROOM_ORIG = {
            x = 0, y = 0, w = G.TILE_W or 16, h = G.TILE_H or 10
        }
    end
...
function create_playing_card(card_init, area, skip_materialize, silent, colours)
    ...
    local card = Card(_area.T.x + _area.T.w/2, _area.T.y, G.CARD_W, G.CARD_H, card_init.front, card_init.center, {playing_card = G.playing_card})
...
function create_card(_type, area, legendary, _rarity, skip_materialize, soulable, forced_key, key_append)
    ...
    local card = Card(_area.T.x + _area.T.w/2, _area.T.y, G.CARD_W, G.CARD_H, front, center, {bypass_discovery_center = area==G.shop_jokers or area == G.pack_cards or area == G.shop_vouchers or (G.shop_demo and area==G.shop_demo) or area==G.jokers or area==G.consumeables,
