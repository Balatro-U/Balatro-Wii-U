...
function GET_DISPLAYINFO(screenmode, display)
  -- Wii U/LovePotion: pouze jeden display, rozlišení 1280x720
  local display_count = 1
  local realw, realh = 1280, 720
  local unscaled_dims = {width = 1280, height = 720}
  local res_option = 1
  ...
  for i = 1, display_count do
    ...
    if screenmode == 'Fullscreen' then
      ...
      G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.strings[1] = '1280 X 720'
...
function play_sound(sound_code, per, vol)
  ...
  if sound_code and G.SETTINGS.SOUND.volume > 0.001 then
    ...
    PLAY_SOUND(G.ARGS.play_sound) -- pouze toto
  end
end
...
function modulate_sound(dt)
  ...
  MODULATE(G.ARGS.push) -- pouze toto
end
...
function set_profile_progress()
  -- Zkontroluj základní struktury
  if not G.PROFILES then
    G.PROFILES = {}
  end
  
  if not G.SETTINGS or not G.SETTINGS.profile then
    G.SETTINGS = G.SETTINGS or {}
    G.SETTINGS.profile = 1
  end
  
  -- Inicializuj profil pokud neexistuje
  if not G.PROFILES[G.SETTINGS.profile] then
  ...
  -- Zkontroluj že challenge_progress existuje
  if not G.PROFILES[G.SETTINGS.profile].challenge_progress then
    G.PROFILES[G.SETTINGS.profile].challenge_progress = {completed = {}}
  end
  
  if not G.PROFILES[G.SETTINGS.profile].challenge_progress.completed then
    G.PROFILES[G.SETTINGS.profile].challenge_progress.completed = {}
  end
  ...
  -- Bezpečná iterace přes P_CENTERS (může být nil)
  if G.P_CENTERS then
    for _, v in pairs(G.P_CENTERS) do
      if v.set == 'Back' and not v.omit then
        G.PROGRESS.deck_stakes.of = G.PROGRESS.deck_stakes.of + (G.P_CENTER_POOLS and G.P_CENTER_POOLS.Stake and #G.P_CENTER_POOLS.Stake or 0)
        G.PROGRESS.deck_stakes.tally = G.PROGRESS.deck_stakes.tally + get_deck_win_stake(v.key)
      end
      if v.set == 'Joker' then 
        G.PROGRESS.joker_stickers.of = G.PROGRESS.joker_stickers.of + (G.P_CENTER_POOLS and G.P_CENTER_POOLS.Stake and #G.P_CENTER_POOLS.Stake or 0)
        G.PROGRESS.joker_stickers.tally = G.PROGRESS.joker_stickers.tally + get_joker_win_sticker(v, true)
      end
    end
  end

  -- Bezpečná iterace přes CHALLENGES (může být nil)
  ...
  -- Bezpečné kopírování výsledků
  if not G.PROFILES[G.SETTINGS.profile].progress then
    G.PROFILES[G.SETTINGS.profile].progress = {}
  end
...
function init_localization()
  ...
  G.localization.misc.v_dictionary = G.localization.misc.v_dictionary or {}
...
function localize(args, misc_cat)
  if args and not (type(args) == 'table') then
    ...
    return (G.localization.misc.v_dictionary and G.localization.misc.v_dictionary[args]) or 'ERROR'
  end
