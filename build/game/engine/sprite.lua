...
G.BRUTE_OVERLAY = G.BRUTE_OVERLAY or {1,1,1,1}
...
function Sprite:init(X, Y, W, H, new_sprite_atlas, sprite_pos)
    if not new_sprite_atlas then
        error("Sprite:init: new_sprite_atlas je nil!")
    end
    if not new_sprite_atlas.px or not new_sprite_atlas.py or not new_sprite_atlas.image then
        error("Sprite:init: new_sprite_atlas nemá px/py/image!")
    end
    ...
    -- Bezpečné vkládání do G.I.SPRITE
    G.I = G.I or {}
    G.I.SPRITE = G.I.SPRITE or {}
...
function Sprite:set_sprite_pos(sprite_pos)
    if not self.atlas then
        error("Sprite:set_sprite_pos: self.atlas je nil!")
    end
    if not self.atlas.px or not self.atlas.py or not self.atlas.image then
        error("Sprite:set_sprite_pos: self.atlas nemá px/py/image!")
    end
...
--G.localization.misc.dictionary = G.localization.misc.v_dictionary