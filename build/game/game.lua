...
function Game:start_up()
    ...
    if settings then 
        ...
        if G.VERSION >= '1.0.0' and false and ((not settings_file.version) or (settings_file.version < '1.0.0')) then  
            for i = 1, 3 do
...
-- Wii U/LovePotion: Save manager thread is not used
    G.SAVE_MANAGER = {}
    boot_timer('savemanager', 'shaders',0.4)

    --Load all shaders from resources nejde na wii u
    --self.SHADERS = {}
    ...
    wiiu_log("Profil bude načten později: " .. (G.SETTINGS.profile or 1))

    self.SETTINGS.QUEUED_CHANGE = {}
    self.SETTINGS.music_control = {desired_track = '', current_track = '', lerp = 1} 

        if self.set_render_settings then
   ...
   self.shared_debuff = {}
    self.shared_soul = {}
    self.shared_undiscovered_joker = {}
    self.shared_undiscovered_tarot = {}
    self.shared_sticker_eternal = {}
    self.shared_sticker_perishable = {}
    self.shared_sticker_rental = {}
    self.shared_stickers = {
        White = {}, Red = {}, Green = {}, Black = {}, Blue = {}, Purple = {}, Orange = {}, Gold = {}
    }
    self.shared_seals = {
        Gold = {}, Purple = {}, Red = {}, Blue = {}
    }
    self.sticker_map = {
        'White','Red','Green','Black','Blue','Purple','Orange','Gold'
    }
 ...
 -- CURSOR není potřeba na Wii U - používáme pouze gamepad ovládání
self.CURSOR = {
    T = {x = 0, y = 0, w = 0, h = 0},
    VT = {x = 0, y = 0, w = 0, h = 0, scale = 0},
    states = {collide = {can = false}, visible = false},
    translate_container = function(self) end,
    draw = function(self) end,  -- Prázdná draw funkce = nekreslí se
    hard_set_VT = function(self) end,
...
for _, v in ipairs(self.FONTS) do
    v.FONT = love.graphics.newFont("standard", 20) -- Wii U system font
end
-- Nastav systémový font jako výchozí pro veškerý text
love.graphics.setFont(self.FONTS[1].FONT)
...
function Game:set_render_settings()
    ...
    --Set fiter to linear interpolation and nearest, best for pixel art nejde na wii u
...
function Game:save_progress()
    ...
    -- Odstraněno: G.SAVE_MANAGER.channel:push
end

function Game:save_notify(card)
    -- Odstraněno: G.SAVE_MANAGER.channel:push
    -- Místo toho pouze logika, pokud potřebuješ
    -- Například:
    -- print("Save notify for card: "..tostring(card.key))
end
...
function Game:prep_stage(new_stage, new_state, new_game_obj)
    ...
    -- Inicializuj ROOM_ORIG pro LovePotion
    if not G.ROOM_ORIG then
        G.ROOM_ORIG = {
            x = self.ROOM_PADDING_W or 0,
            y = self.ROOM_PADDING_H or 0,
            w = self.TILE_W or 16,
            h = self.TILE_H or 10
        }
    end
...
function Game:main_menu(change_context) --True if main menu is accessed from the splash screen, false if it is skipped or accessed from the game
    ...
    if (not G.SETTINGS.tutorial_complete) and G.SETTINGS.tutorial_progress and G.SETTINGS.tutorial_progress.completed_parts and G.SETTINGS.tutorial_progress.completed_parts['big_blind'] then
    -- sem patří tvůj kód, např. zobrazení tutoriálu
    end
...
function Game:update(dt)
   ...
   if not G.SETTINGS.tutorial_complete then 
        if G.FUNCS.tutorial_controller then
            pcall(G.FUNCS.tutorial_controller)
        else
            -- Tutorial controller neexistuje - pravděpodobně není potřeba na Wii U
            wiiu_log("tutorial_controller funkce neexistuje")
        end
    end
    modulate_sound(dt)
    ...
    -- Wii U/LovePotion: Save manager thread is not used, saving is handled differently
    if G.FILE_HANDLER and G.FILE_HANDLER.update_queued then
