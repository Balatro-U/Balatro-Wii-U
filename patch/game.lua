...
function Game:start_up()
    ...
    -- WII U SPECIFIC: Force 1x textures for memory optimization
    if love.system.getOS() == "cafe" then
        self.SETTINGS.GRAPHICS.texture_scaling = 1
        print("[WII U] Forcing 1x texture scaling for memory optimization")
    elseif self.SETTINGS.GRAPHICS.texture_scaling then
    ...
    -- Shaders: PC loads .fs; Wii U uses precompiled CafeGLSL .gsh binaries
    if love.system.getOS() ~= "cafe" then
        ...
        -- Wii U: load effect shaders from /vol/content/shaders/*.gsh generated by CafeGLSL
        local function try_load_gsh(name)
            local ok, shader = pcall(love.graphics.newShader, "/vol/content/shaders/" .. name .. ".gsh")
            if ok and shader then
                self.SHADERS[name] = shader
            end
        end
        -- Core effects used by splash/menu and postprocess
        try_load_gsh('splash')
        try_load_gsh('flash')
        try_load_gsh('dissolve')
        try_load_gsh('vortex')
        try_load_gsh('CRT')
    end
    -- Expose shaders globally (may be empty on Wii U; engine provides defaults)
    G.SHADERS = self.SHADERS
    ...
    print("[DEBUG] About to create shared_debuff sprite")
    print("[DEBUG] ASSET_ATLAS['centers']: " .. tostring(self.ASSET_ATLAS["centers"]))
    if self.ASSET_ATLAS["centers"] then
        print("[DEBUG] ASSET_ATLAS['centers'].px: " .. tostring(self.ASSET_ATLAS["centers"].px))
    end
...
function Game:set_language()
    ...
    -- Keep platform-agnostic font metrics to avoid oversized text on Wii U
...
function Game:set_render_settings()
    print("[DEBUG] set_render_settings() called")
    -- WII U SPECIFIC: Force 1x textures to save memory on Wii U
    if love.system.getOS() == "cafe" then
        self.SETTINGS.GRAPHICS.texture_scaling = 1
        print("[WII U] Forcing 1x texture scaling for memory optimization")
    else
    ...
    print("[DEBUG] Loading animation_atli, count: " .. #self.animation_atli)
    for i=1, #self.animation_atli do
        print("[DEBUG] Loading animation_atli[" .. i .. "]: " .. self.animation_atli[i].name)
    ...
    for i=1, #self.asset_atli do
        print("[DEBUG] Loading asset_atli[" .. i .. "]: " .. self.asset_atli[i].name)
    ...
    for i=1, #self.asset_images do
        print("[DEBUG] Loading asset_images[" .. i .. "]: " .. self.asset_images[i].name)
    ...
    print("[DEBUG] set_render_settings() completed successfully")
    local atlas_keys = {}
    for k, v in pairs(self.ASSET_ATLAS) do
        table.insert(atlas_keys, k)
    end
    print("[DEBUG] ASSET_ATLAS keys: " .. table.concat(atlas_keys, ", "))
end
...
function Game:init_window(reset)
    ...
    -- Keep global TILESIZE/TILESCALE defaults to preserve intended layout and text scale on all platforms
    ...
    G.E_MANAGER:add_event(Event({
        func = (function()
            ...
            G.SPLASH_BACK:define_draw_steps({{
                shader = 'splash',
                send = {
                                        {name = 'time', ref_table = G.TIMERS, ref_value = 'TOTAL'},
            ...
            --spawn in splash card (use REAL timer, non-blocking, priority queue)
            local SC = nil
            G.E_MANAGER:add_event(Event({
                trigger = 'after',
                delay = 0.2,
            ...
            --dissolve fool card and start to fade in the vortex (REAL timer, non-blocking, priority queue)
            G.E_MANAGER:add_event(Event({
                trigger = 'after',
                delay = 1.8,
                blocking = false,
...
--when faded to white, spit out the 'Fool's' cards and slowly have them settle in to place
        ...
        -- Fallback: provide a safe stub for set_screen_positions if it's not defined elsewhere
        if type(set_screen_positions) ~= 'function' then
        function set_screen_positions()
            if not G then return end

            -- Determine canvas size
            local cw, ch = 1280, 720
            if G.CANVAS and G.CANVAS.getWidth then
                cw, ch = G.CANVAS:getWidth(), G.CANVAS:getHeight()
            end

            -- Ensure AA canvas exists and matches size
            if love and love.graphics and love.graphics.newCanvas then
                local need_new = false
                if not G.AA_CANVAS then
                    need_new = true
                elseif G.AA_CANVAS.getWidth and (G.AA_CANVAS:getWidth() ~= cw or G.AA_CANVAS:getHeight() ~= ch) then
                    need_new = true
                end
                if need_new then
                    local ok, c = pcall(function() return love.graphics.newCanvas(cw, ch) end)
                    if ok and c then G.AA_CANVAS = c end
                end
            end

            -- Compute canvas scale to fit screen
            if love and love.graphics and love.graphics.getWidth then
                local sw, sh = love.graphics.getWidth(), love.graphics.getHeight()
                if cw > 0 and ch > 0 then
                    G.CANV_SCALE = math.min(sw / cw, sh / ch)
                else
                    G.CANV_SCALE = 1
                end
            else
                G.CANV_SCALE = G.CANV_SCALE or 1
            end
        end
        end
...
function Game:main_menu(change_context) --True if main menu is accessed from the splash screen, false if it is skipped or accessed from the game
    ...
    --VERSION
    do
        local is_wiiu = (love.system.getOS() == "cafe")
        local version_scale = is_wiiu and 0.08 or 0.18
        local version_offset = is_wiiu and {x = -0.4, y = 0.4} or {x = -0.6, y = 0.6}
        UIBox{
            definition = 
            {n=G.UIT.ROOT, config={align = "cm", colour = G.C.UI.TRANSPARENT_DARK}, nodes={
                {n=G.UIT.T, config={text = G.VERSION, scale = version_scale, colour = G.C.UI.TEXT_LIGHT}}
            }}, 
            config = {align="tri", offset = version_offset, major = G.ROOM_ATTACH, bond = 'Weak'}
        }
    end
end
...
love.graphics.pop()
    ...
    -- Safety check for CRT shader with complete shader setup (attempt on all platforms, fallback on failure)
