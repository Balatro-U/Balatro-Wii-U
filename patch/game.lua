...
function Game:start_up()
    ...
    if love.system.getOS() == "cafe" then  -- Wii U platform
        print("[WII U] Loading converted .gsh shaders")
        -- Manually load converted shaders that we know exist (using real shader names)  
        local wii_u_shaders = {
            "background", "booster", "CRT", "debuff", "dissolve", "flame", "flash", 
            "foil", "gold_seal", "holo", "hologram", "negative", 
            "negative_shine", "played", "polychrome", "skew", "splash", "vortex", "voucher"
        }
        
        for _, shader_name in ipairs(wii_u_shaders) do
            print("[WII U] Attempting to load shader: " .. shader_name)
            
            -- Try to load shader with timeout protection
            local success, shader_or_error = pcall(function()
                -- Force aggressive garbage collection before each shader load
                collectgarbage("collect")
                collectgarbage("collect")  -- Double collection for safety
                return love.graphics.newShader("/vol/content/shaders/game/"..shader_name..".gsh")
            end)
            
            if success and shader_or_error then
                self.SHADERS[shader_name] = shader_or_error
                print("[WII U] Successfully loaded shader: " .. shader_name)
            else
                print("[WII U] Failed to load shader: " .. shader_name .. " - Error: " .. tostring(shader_or_error or "unknown"))
            end
            
            -- Longer delay to prevent memory issues
            love.timer.sleep(0.1)
        end
        
        -- Count loaded shaders properly
        local shader_count = 0
        for k, v in pairs(self.SHADERS) do
...
function Game:set_language()
    if not self.LANGUAGES then 
        ...
        -- WII U SPECIFIC: Increase font sizes for better visibility
        if love.system.getOS() == "cafe" then
            print("[WII U] Adjusting font sizes for better visibility")
            for _, v in ipairs(self.FONTS) do
                v.render_scale = math.max(v.render_scale * 2.5, 50)  -- Make fonts much larger with minimum size
                v.FONTSCALE = math.max(v.FONTSCALE * 2.0, 0.3)       -- Also increase font scale with minimum
            end
        end
        for _, v in ipairs(self.FONTS) do
